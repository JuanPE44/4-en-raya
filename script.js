...
class Tablero {
  ...
  recorrerTablero() {
    ...
    for (let i = 1 - this.tablero.length; i < this.tablero.length; i++) {
      for (
        let x = -Math.min(0, i), y = Math.max(0, i);
        x < this.tablero.length && y < this.tablero.length;
        x++, y++
      ) {
  ...
  cuatroIguales(casillas) {
    ...
    casillas.forEach((e) => {
      ...
      if (XO === XOant && XOant !== "") {
        if (this.jugador.puntaje === 1) {
          this.jugador.ganarArray.push(eAnt);
        }
        ...
        if (this.jugador.puntaje === 4) {
          ...
          this.jugador.elementoTxt.innerHTML =
            this.jugador.txt + this.jugador.puntajeTotal;
